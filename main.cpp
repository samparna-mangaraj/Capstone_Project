...
void printPermissions(const std::string &perm, const std::string &name)
{
    ...
    if (perm[0] == 'd')
        color = BLUE;
    else if (perm.find('w') != std::string::npos)
        color = GREEN;
    else
        color = YELLOW;
...
void makeDir(const std::string &name)
{
    if (fs::create_directory(name))
    {
        ...
        std::cout << RED << "Failed to create directory.\n"
                  << RESET;
}
...
void removeDir(const std::string &name)
{
    ...
    if (fs::is_empty(name))
    {
        ...
        std::cout << YELLOW << "Directory not empty.\n"
                  << RESET;
}
...
void chmodFile(const std::string &name, const std::string &permCode)
{
    ...
    if (permCode.size() != 3)
    {
        std::cout << YELLOW << "Use format like 755.\n"
                  << RESET;
        return;
    }
...
int main()
{
    ...
    while (true)
    {
        ...
        if (cmd == "exit")
            ...
            copyFileCmd(arg1, arg2);
        else if (cmd == "mv")
            moveFileCmd(arg1, arg2);
        else if (cmd == "help")
            showHelp();
        else if (cmd == "sudo")
        {
